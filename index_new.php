...
foreach (['PHONE', 'EMAIL', 'NAME', 'INSTA'] as $key) {
	switch ($key) {
		...
		case 'EMAIL':
			...
			break;
		case 'NAME':
			if ($entityTypeId != 4) { // В Компаниях нет полей с именем
			...
			break;
		case 'INSTA':
			$instaAcc = parseInstaAcc($crmEntity, $entityTypeId);
...
// Тут магия, автоматический поиск дубликатов битрикс24 удаляет лид как раз к тому моменту когда этот скрипт завершает свою работу
// Поэтому нужно ещё раз получить нужную сущность и проверить не удалена ли она
sleep(3);
$try = getCrmEntity($_REQUEST['ID'], $entityTypeId);
...
// Создание поста в ленту notifyPost забраковали, но я не стал тратить время на удаление функций
