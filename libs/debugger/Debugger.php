...
class Debugger extends Exception {
	...
	*/
    ...
    /**
	* Дополнительная функция логгирования (параметр с путем файла прописан в либе)
	* @var $requestParams      - параметры запросы
	* @var str $result  - ответ от сервера
	* @var str $title - наименование лога
	*/
	public static function writeToLogSet($requestParams, $result, $title = 'DEBUG', $path = PREARRANGED_PATH, $bool = true) {
		if ($bool) {
			$log = "\n--------------------\n";
			$log .= date('d.m.Y H:i:s')."\n";
			$log .= "Caption: ".$title."\n";
			if ($requestParams != null or $result != null) {
				$log .= "Параметры запроса:\n" . print_r($requestParams, 1) . "\n\nОтвет от сервера:\n" . print_r($result, 1);
			}
			$log .= "\n--------------------\n";
			file_put_contents($path, $log, FILE_APPEND);
		}
