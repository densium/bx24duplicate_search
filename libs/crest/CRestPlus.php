...
class CRestPlus extends CRest {
	...
	*/
	private static function lightCurl ($arParams, $wannaCurrentUser = true) {
		...
		if ($arSettings !== false) {
			$url = $arSettings[ "client_endpoint" ] . $arParams[ 'method' ] . '.' . parent::TYPE_TRANSPORT;
			...
			try {
				...
				if(!empty($result[ 'error' ])) $result[ 'error_information' ] = 'ERROR';
	...
	*/
	public static function callCurrentUser () {
		...
		$arPostAdmin = array('method' => 'user.admin','params' => array());
		if(defined('C_REST_CURRENT_ENCODING')) $arPostAdmin[ 'params' ] = parent::changeEncoding($arPostAdmin[ 'params' ]);
		$admin = static::lightCurl($arPostAdmin);

		$result = array('admin' => $admin['result'],'user'  => $user['result']);
		return $result;
	}
